...
wprm_admin.delete_or_merge_term = function(term_id, taxonomy, new_term_id) {
	...
	jQuery.post(wprm_manage.ajax_url, data, function() {
		jQuery('.wprm-manage-datatable').DataTable().ajax.reload();
	});
};
...
jQuery(document).ready(function($) {
	...
	var wprm_datatable = jQuery('.wprm-manage-datatable').DataTable( {
		...
		drawCallback: function() {
			...
			jQuery('.wprm-manage-ingredients-actions').tooltipster({
				content: '<div class="wprm-manage-ingredients-actions-tooltip"><div class="tooltip-header">&nbsp;</div><a href="#" class="wprm-manage-ingredients-actions-link">Edit Ingredient Link</a><a href="#" class="wprm-manage-ingredients-actions-merge">Merge into Another Ingredient</a></div>',
				contentAsHTML: true,
				...
				functionReady: function(instance, helper) {
					...
					if(count == 0) {
						jQuery(helper.tooltip)
							.find('.wprm-manage-ingredients-actions-merge')
							.removeClass('wprm-manage-ingredients-actions-merge')
							.addClass('wprm-manage-ingredients-actions-delete')
	...
	jQuery(document).on('click', '.wprm-manage-ingredients-actions-link', function() {
		...
		if(new_link) {
			link_container.html('<a href="' + new_link + '" target="_blank">' + new_link + '</a>');
