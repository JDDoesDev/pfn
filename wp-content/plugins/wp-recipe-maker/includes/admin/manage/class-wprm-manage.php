...
class WPRM_Manage {
	...
	public static function ajax_update_term_metadata() {
		if ( check_ajax_referer( 'wprm', 'security', false ) ) {
			...
			$value = isset( $_POST['value'] ) ? sanitize_text_field( wp_unslash( $_POST['value'] ) ) : ''; // Input var okay.

			// Check if valid field.
			if ( $term_id && in_array( $field, array( 'ingredient_link' ), true ) ) {
	...
	public static function add_submenu_page() {
		add_submenu_page( 'wprecipemaker', __( 'Manage', 'wp-recipe-maker-premium' ), __( 'Manage', 'wp-recipe-maker-premium' ), 'manage_options', 'wprecipemaker', array( __CLASS__, 'page_template' ) );
	}
